...
This document is the source-of-truth contract that the Python bridge service
(running Qiskit Metal) must implement. The frontend in this repository ships
the typed client and UI for these endpoints. **No Python is implemented in
this project.**

> Configuration: the frontend reads the bridge base URL from
> `import.meta.env.VITE_BRIDGE_URL`. When unset, the UI shows a
> "Bridge not configured" empty state — it never falls back to mock data.
...
`hint` coordinates are in micrometers relative to the component's origin at
its default parameter values, used by the editor to position pin handles
