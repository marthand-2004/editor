...
export function CodePanel() {
  ...
  const cached = useQuery<GeneratedCode | undefined>({
    queryKey: ["bridge", "generate-code"],
    queryFn: () => undefined,
    enabled: false,
    staleTime: Infinity,
  });
  ...
  const code = mu.data?.code ?? cached.data?.code ?? "";
  const filename = mu.data?.filename ?? cached.data?.filename ?? "design.py";
