...
export function EditorCanvas() {
  ...
  const renderQ = useQuery({
    ...
    enabled: false,   // Only runs when toolbar Render button invalidates the query
    staleTime: Infinity,
  });
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    // Fetch metadata to seed default params from the bridge (cached).
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full overflow-hidden bg-background">
      ...
      >
        ...
        {/* Bridge-rendered design (full design SVG). */}
        {renderQ.data?.svg && (
          <BridgeRender
            result={renderQ.data}
            state={state}
        ...
        {/* Per-placement previews remain visible beneath a full-design render. */}
        {state.placements.map((p) => (
          <PlacementPreview key={p.id} placement={p} worldToScreen={worldToScreen} />
        ))}

        {/* Placement glyphs (selection halo + name + pin handles). */}
        ...
        {/* Schematic connections (drawn as straight dashed lines until bridge renders the real route). */}
...
function BridgeRender({
  ...
  // The bridge returns absolute geometry in design coordinates (um).
  // The frontend already knows world-position per placement (mm), so we render
  // the full design SVG once at the world origin and let the editor's pan/zoom
  // transform handle viewport math. Per-connection route SVGs are overlaid in
  // the same coordinate space so swap-on-edit is cheap.
  ...
  return (
    <g
      ...
      dangerouslySetInnerHTML={{
        __html:
          result.svg +
          result.routes.map((r) => r.svg).join(""),
      }}
    />
  );
}
...
function PlacementPreview({
  ...
  // Bridge returns units: "um". MM_TO_PX = 80px/mm. UM_TO_MM = 0.001mm/um.
  // So 1um = 0.001mm * 80px/mm = 0.08px at zoom=1.
  const unitToMm = preview.units === "um" ? UM_TO_MM : 1;
  const scale = state.zoom * MM_TO_PX * unitToMm;

  const vb = preview.viewBox;

  // The viewBox center in component coordinates:
  const vbCenterX = vb.x + vb.w / 2;
  const vbCenterY = vb.y + vb.h / 2;

  // The SVG element top-left must be offset so its center lands at (px, py).
  // Y is negated because SVG Y-down conflicts with our canvas Y-up convention.
  const svgLeft = px - vbCenterX * scale;
  const svgTop = py + vbCenterY * scale - vb.h * scale;  // flip Y
  ...
  return (
    <g>
      ...
      >
        <g dangerouslySetInnerHTML={{ __html: preview.svg }} />
      </svg>
    </g>
  );
}
...
function PlacementGlyph({
  ...
  const { px, py } = worldToScreen(placement.x, placement.y);
  // Default-size hit area when no preview info — 0.5mm box.
  const sizePx = Math.max(28, 0.5 * MM_TO_PX * zoom);
  const half = sizePx / 2;
  ...
  return (
    ...
    >
      {/* Visible placeholder when no SVG preview is loaded yet. */}
      <rect
        x={-half}
        y={-half}
        width={sizePx}
        height={sizePx}
        fill="color-mix(in oklab, var(--primary) 6%, transparent)"
